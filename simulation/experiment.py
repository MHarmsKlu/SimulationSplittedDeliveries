...
def initialize_sim(kpi, war_config, sim_config, test_param, test_value):
    if test_param in war_config.keys():
        war_config[test_param]= test_value
    ...
    war_config['kpi'] = kpi
    sim_config["warehouse"] = Warehouse(war_config)
    return Simulation( config=sim_config)

def experiment(KPIs,war_config, sim_config,test_param,init_value, step_size, max_value, target_params):
