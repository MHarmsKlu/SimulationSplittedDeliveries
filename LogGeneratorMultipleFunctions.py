...
def generate_delivery_schedule(time_period, order_quantity, del_func, time_slots_del, seed=None):
    """
    ...
    for day in range(1, time_slots_del):
        expected_delivery = distribute_values(del_func, time_slots_del, order_quantity, fixed_values=delivery_schedule)
...
def simulate_deliveries(order_quantity, time_period, start_date, del_func, time_slots_del, pid, seed=None):
    """
    ...
    if seed:
        np.random.seed(seed)
    delivery_schedule = generate_delivery_schedule(time_period, order_quantity, del_func, time_slots_del, seed)
    ...
    for day, qty in delivery_schedule.items():
        if qty > 0:
            total_delivery += qty
            events.append((pid, 'Delivery', start_date + pd.Timedelta(days=day-1), qty))
